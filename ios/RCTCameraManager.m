        ...
        [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:[self.stillImageOutput connectionWithMediaType:AVMediaTypeVideo] completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
            ...
            if (imageDataSampleBuffer) {
                ...
                if ([options objectForKey:@"rotation"]) {
                    ...
                    face = [self getFaces:ciImage orientation:[NSNumber numberWithInt:rotation]];
                    ...
                    face = [self getFaces:ciImage orientation:[NSNumber numberWithInt:metadataOrientation]];
                    bool rotated = false;
                ...
                [self saveImage:rotatedImageData target:target metadata:imageMetadata resolve:resolve reject:reject additionalData:face];
...
- (NSDictionary *)getFaces:(CIImage*)myImage orientation:(NSNumber*)orientation{
    ...
    NSArray *features = [detector featuresInImage:myImage options:imageOptions];
    NSDictionary *event = @{};
    for (CIFaceFeature *f in features) {
        
        if(f.hasLeftEyePosition && f.hasMouthPosition && f.hasRightEyePosition){
            event = @{
                      @"type": f.type,
                      @"bounds": @{
                              @"origin": @{
                                      @"x": [NSString stringWithFormat:@"%f", f.bounds.origin.x],
                                      @"y": [NSString stringWithFormat:@"%f", f.bounds.origin.y]
                                      },
                              @"size": @{
                                      @"height": [NSString stringWithFormat:@"%f", f.bounds.size.height],
                                      @"width": [NSString stringWithFormat:@"%f", f.bounds.size.width],
                                      }
                              }
                      };
        }
...
- (void)saveImage:(NSData*)imageData target:(NSInteger)target metadata:(NSDictionary *)metadata resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    [self saveImage:imageData target:target metadata:metadata resolve:resolve reject:reject additionalData:@{}];
}


- (void)saveImage:(NSData*)imageData target:(NSInteger)target metadata:(NSDictionary *)metadata resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject additionalData:(NSDictionary*)additionalData {
    ...
    resolve(@{@"path":responseString, @"additionalData": additionalData});
}
